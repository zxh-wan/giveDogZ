...
object ToastUtil {
    ...
    private var yOffset = GenApp.instance.dp2px(64f)
    ...
    private fun show(@StringRes resId: Int, duration: Int) {
        show(GenApp.instance.resources.getText(resId).toString(), duration)
    }

    private fun show(@StringRes resId: Int, duration: Int, vararg args: Any) {
        show(String.format(GenApp.instance.resources.getString(resId), args), duration)
    }
    ...
    private fun showed(text: CharSequence, duration: Int) {
        HANDLER.post({
            cancel()
            sToast = Toast.makeText(GenApp.instance, text, duration)
    ...
    private fun getView(@LayoutRes layoutId: Int): View {
        ...
        val inflate = GenApp.instance.getSystemService(Context.LAYOUT_INFLATER_SERVICE) as LayoutInflater
